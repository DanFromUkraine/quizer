...
export function getSelector(testId: string) {
    return (locator: Page | Locator) => {
        if (typeof locator === 'undefined')
            throw new Error(`Locator for testid ${testId} is undefined.`);
...
export function getAddElementInListWithSuccessExpectations({
    ...
    return async (scope: Page | Locator) =>
        await test.step(testStepTitle, async () => {
            const initialCount = await getItemLocator(scope).count();
            const actionButton = getAddButton(scope);
            await actionButton.click();

            await expect(getItemLocator(scope)).toHaveCount(initialCount + 1);
        });
}
...
export function getRemoveElFromTheListWithSuccessExpectations({
    ...
    return async (scope: Page | Locator, elIndexToDelete: number) => {
        await test.step(testStepTitle, async () => {
            const initElsCount = await getItemLocator(scope).count();
            const targetEl = getItemLocator(scope).nth(elIndexToDelete);
            const deleteBtnEl = getDeleteBtnEl(targetEl);
            await deleteBtnEl.click();
            await expect(getItemLocator(scope)).toHaveCount(initElsCount - 1);
        });
    };
}
...
export async function swipeOption({
    ...
    if (optionBox === null)
        throw new Error(`Couldn't swipe option in ${direction} direction`);
    ...
    const endX =
        direction === 'left' ? optionBox.x + optionBox.width : optionBox.x;
    ...
    try {
        ...
        await test.step('Move element via touch', async () => {
            for (let i = 1; i <= steps; i++) {
                ...
                if (i < steps) {
                    await new Promise((r) =>
                        setTimeout(r, Math.round(interval))
                    );
                }
            }
        });
