...
export async function updateCardAtomHelper({
    ...
    if (optionIdsToInsert.length > 0) {
        ...
        newOptionIds = getListWhereNoSuchIds(optionIds, optionIdsToDelete);
    }
...
export async function updateShortCardsOnlyAtomHelper({
    ...
    if (areAnyNew) {
        newCardIdsOrder = getListWithSuchIds(cardIdsOrder, cardIdsToInsert);
        newShortCardIds = getListWithSuchIds(shortCardIds, cardIdsToInsert);
    }

    if (areAnyDeleted) {
        newCardIdsOrder = getListWhereNoSuchIds(cardIdsOrder, cardIdsToDelete);
        newShortCardIds = getListWhereNoSuchIds(shortCardIds, cardIdsToDelete);
    }
