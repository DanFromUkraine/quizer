...
export function getNumOfChoicesCalculator<T extends { currentValue: unknown }>(
    ...
    targetAtomFamily: AtomFamily<string, PrimitiveAtom<T> & WithInitialValue<T>>
) {
    return (ids: string[]) =>
        atom((get) => {
            let count = 0;
...
const getCalcNumOfChoicesAtom = ({
    ...
    atom((get) => {
        const expCardChoicesCount = get(
            getCalcNumOfChoicesInExpCardStoriesAtom(explicitCardStoryIds)
        );
        const typeInCardChoicesCount = get(
            getCalcNumOfChoicesInTypeInCardStoriesAtom(typeInCardStoryIds)
        );
        const isCorrectCardChoicesCount = get(
            getCalcNumOfChoicesInIsCorrectCardStoriesAtom(isCorrectCardStoryIds)
        );
...
export const getIfExpStoryCardCorrectAtom = (cardId: string) =>
    atom((get) => {
        const { currentValue } = get(explicitCardStoriesAtomFamily(cardId));
...
export const getAssociationsForBookAtomOnlyIncomplete = (bookId: string) =>
    atom((get) => {
        ...
        const associationsForBook: string[] = allAssociations[bookId] || [];
...
export const getNumOfCorrectOptions = (storyCardId: string) =>
    atom((get) => {
        const { options } = get(explicitCardStoriesAtomFamily(storyCardId));
