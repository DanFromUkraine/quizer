...
- Keep changes aligned with the existing feature-folder style. Prefer colocated `description.tsx`, `RenderNavLinks.tsx`, `UI.tsx`, selectors, steps, and constants where that pattern already exists.
...
- Keep client-only logic in `RenderNavLinks.tsx` files or components with the appropriate client boundary.
